...
## דשבורד מקומי — `Dashboard.html`

בנוסף לאפליקציה בנה בניתי דשבורד עצמאי בקובץ HTML אחד, שקורא ישירות מ-Airtable
דרך ה-REST API. הוא שימושי לבדיקה מהירה של הנתונים בלי לפתוח את האפליקציה:
ארבעה כרטיסי מדדים, ארבע טבלאות עם חיפוש וסינון, והכול בעברית RTL.

הוא דורש טוקן קריאה (`data.records:read`) שנשמר ב-`localStorage` של הדפדפן
בלבד, ולכן **מיועד להרצה מקומית**:

```
python -m http.server 8788 --bind 127.0.0.1 --directory app
```

ואז לפתוח `http://localhost:8788/Dashboard.html`.
