<div dir="rtl" align="right">

# ERP-AI — מערכת ניהול עסק חכמה על n8n

פרויקט גמר · איי.איי אלקטרוניקה (AI Electronics) · n8n + סוכני AI + RAG + Airtable + Base44

**🌐 [לתצוגת הפרויקט](https://shaiautomation-oss.github.io/Final_Pro/)**

---

## מה בניתי

מערכת ERP לעסק אלקטרוניקה ישראלי, שבה **סוכני בינה מלאכותית ותהליכים אוטומטיים
עושים את רוב העבודה**: עונים ללקוחות בטלגרם, שולחים מיילי מכירות, מחשבים מע"מ,
מפיקים מסמכי חשבונית ומנתחים את העסק — בלי שורת קוד אחת.

בניתי אותה בשלוש שכבות:

| שכבה | במה מימשתי | תפקיד |
|---|---|---|
| **נתונים** | Airtable — בסיס `FINAL` | 4 טבלאות, מקור האמת לכל הרשומות |
| **לוגיקה ואוטומציה** | n8n Cloud | 10 תהליכים + 3 סוכני AI + מאגר וקטורי |
| **ממשק** | Base44 | אפליקציית ווב: דשבורד, טבלאות, טפסים וצ'אט |

---

## עשרת התהליכים

| # | מה עושה | טריגר |
|---|---|---|
| WF1 | אימות מסמכי מס והכנסה לתור הפקה | חשבונית חדשה |
| WF3 | קליטת לידים וסינון כפילויות | Webhook |
| WF4a | סוכן מכירות — מיילים קרים | כל 3 שעות |
| WF4b | סוכן מכירות — זיהוי תשובות | כל 30 דקות (Gmail) |
| WF5 | סוכן שירות לקוחות | בוט טלגרם #2 |
| WF6 | מדיניות ← מאגר וקטורי | ידני |
| WF7 | מוצרים ← מאגר וקטורי | ידני |
| WF8 | הפקת מסמך חשבונית והעלאה לדרייב | כל דקה |
| WF9 | סוכן המנהל | בוט טלגרם #1 |
| WF13 | Webhook לאפליקציה | POST מ-Base44 |

**98 צמתים, אפס צמתי קוד.** מה שבפרויקטים אחרים נכתב בקוד מימשתי בצמתים
מוכנים: חישוב מע"מ ב-Edit Fields, ספירת כפילויות ב-Summarize, סיכום הכנסות
ב-Summarize, החלטות ב-IF, ניתוב ב-Switch, והפקת קובץ ב-Convert to File.

**הכול בעברית:** שמות התהליכים, שמות 84 הצמתים, תיאור מתחת לכל צומת, פתקי
הסבר על הקנבס, הנחיות שלושת הסוכנים ותבנית החשבונית.

---

## שלושת הסוכנים

| סוכן | איפה | מה יודע | מגבלה שהגדרתי |
|---|---|---|---|
| **מנהל** (WF9) | בוט טלגרם #1 | הכנסות, מספר חשבוניות, סכומים שלא שולמו | בלי כלים · עד 100 חשבוניות · רק ל-Chat ID של הבעלים |
| **שירות לקוחות** (WF5) | בוט טלגרם #2 | מדיניות וקטלוג מוצרים דרך RAG | לא מאשר החזרים ולא נותן ייעוץ משפטי |
| **מכירות** (WF4a/b) | ברקע | כותב מיילים קרים, מזהה תשובות | ליד אחד בכל הרצה |

---

## מפת התיקייה

```
ERP-AI/
├── workflows/                     10 קובצי n8n מוכנים לייבוא
├── data/
│   ├── policies/                  12 מסמכי מדיניות — מקור ה-RAG (WF6)
│   └── products/products.csv      34 מוצרים — מקור ה-RAG (WF7)
├── index.html                     עמוד התצוגה של הפרויקט (GitHub Pages)
├── app/base44-prompt.md           שני הפרומטים שמהם נבנתה האפליקציה
├── screenshots/                   תיעוד ויזואלי של המערכת בפעולה
└── docs/
    ├── 01-setup.md                כך הקמתי את המערכת
    ├── 02-data-schema.md          סכימת Airtable ומילון סטטוסים
    ├── 03-workflows.md            מפרט מלא של עשרת התהליכים
    ├── 05-telegram-bots.md        שני הבוטים והפרדת ההרשאות
    ├── 06-tax-and-vat.md          מע"מ ומסמכי מס בישראל
    └── 07-end-to-end-tests.md     הבדיקות שביצעתי ותוצאותיהן
```

---

## הרצה מחדש

מי שרוצה להקים את המערכת אצלו מתחיל מ-[docs/01-setup.md](docs/01-setup.md).
בקצרה: חיבורים ב-n8n (Airtable, OpenAI, Gmail, Drive, שני בוטי טלגרם) ←
ייבוא עשרת קובצי ה-JSON ← הזנת ה-Chat ID של הבעלים ב-WF9 וב-WF4b ← הרצה
ידנית של WF6 ואז WF7 ← הפעלת שמונת התהליכים הנותרים ← חיבור כתובת ה-Webhook
של WF13 לאפליקציה.

---

## החלטות תכנון ומגבלות מכוונות

בחרתי לשמור על פשטות במקומות שבהם פתרון מלא היה דורש תשתית נוספת:

- **המאגר הווקטורי יושב בזיכרון** של n8n ונבנה מחדש בהרצת WF6 + WF7. כך אין
  צורך במסד וקטורי חיצוני.
- **החשבונית נשמרת כ-HTML בדרייב** ולא כ-PDF, כדי לא להקים שירות המרה. ההמרה
  ל-PDF היא לחיצה ידנית אחת.
- **קשרים בין טבלאות הם מפתחות זרים כטקסט** (למשל `CUST-0001`) ולא קישורי
  Airtable — פשוט יותר לכתיבה מ-n8n ולהצגה באפליקציה.
- **WF4a שולח ליד אחד בכל הרצה**, כדי שטעות בהגדרה לא תשלח עשרות מיילים.
- **לסוכן המנהל אין כלים** והוא רואה עד 100 חשבוניות. כל המספרים מחושבים
  לפניו בצמתי Summarize, כך שאין לו מקום להמציא נתונים.
- **מספור החשבוניות עלול להתנגש** אם שתי חשבוניות נוצרות באותה דקה. פתרון
  מלא היה דורש טבלת מונים עם נעילה.

---

## המערכת בפעולה

### החשבונית שהופקה אוטומטית
![חשבונית מס INV-2026-0002](screenshots/01-invoice.png)

### סוכן שירות הלקוחות — בוט טלגרם #2
![שיחה עם בוט הלקוחות](screenshots/02-customer-bot.png)

### סוכן המנהל — בוט טלגרם #1
![שיחה עם בוט המנהל](screenshots/03-manager-bot.png)

### אפליקציית הניהול
![דשבורד אפליקציית הניהול](screenshots/04-app-dashboard.png)

### הנתונים ב-Airtable
![טבלת החשבוניות](screenshots/05-airtable.png)

### קנבס האוטומציה ב-n8n
![קנבס WF1](screenshots/06-canvas.png)

### הרצות במערכת
![מסך Executions](screenshots/07-executions.png)

---

## אבטחה

מפתח ה-Airtable אינו נמצא באפליקציה ואינו נמצא בקוד שרץ בדפדפן. הוא יושב רק
ב-Credential של n8n, והאפליקציה מדברת עם המערכת דרך ה-Webhook של WF13 בלבד —
כך שכל הכללים נאכפים במקום אחד.

הפרדתי בין שני בוטי טלגרם: בוט המנהל מוגן בתנאי שמשווה את מזהה הצ'אט לזה של
הבעלים, וכל פנייה אחרת מקבלת סירוב בלי נתונים.

אין בפרויקט שום קובץ שמכיל מפתח, טוקן או סוד. הגישה לנתונים אפשרית רק דרך
החיבורים המוגדרים ב-n8n.

</div>
